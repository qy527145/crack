...
def search(name=None, show_code=False):
    ...
    if show_code:
        for plugin in plugins:
            yield str(plugin['id']), plugin['name'], plugin['pricingModel'], get_plugin_code(plugin['id'])
    else:
        for plugin in plugins:
...
def get_plugin_code(plugin_id):
    plugin_info = requests.get(f'https://plugins.jetbrains.com/api/plugins/{plugin_id}').json()
    if plugin_info.get('purchaseInfo'):
        return plugin_info['purchaseInfo'].get('productCode')


def get_plugin_codes(plugin_ids):
    reqs = [grequests.get(f'https://plugins.jetbrains.com/api/plugins/{plugin_id}') for plugin_id in plugin_ids]
