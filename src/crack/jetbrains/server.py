...
def ensure_key():
    if not os.path.exists("ls.pem"):
        ...
        rsa2.dump_cert(
            ...
            issuer_name="License Servers CA",
            cert_path="ls_cert.crt",
        )
    else:
...
rsa3 = CryptoPlus.load("""-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
""")
...
@app.get("/lservice/rpc/obtainLicense.action")
async def obtain_license(req: Request, salt=""):
    print(req.query_params)
    data = rsa3.encrypt_by_private_key(b"", padding_mode=0)
    data_parts = []
    while len(data) > 0:
        part = data[:64]
        data = data[64:]
        data_parts.append(part)
    data = (b"\x00" + b"\x00".join(data_parts)).hex().upper()
    xml_content = f"""<ObtainLicenseResponse><action>NONE</action><assets><EncodedAsset><data>{data}</data></EncodedAsset></assets><message></message><responseCode>OK</responseCode><salt>{salt}</salt><validationDeadlinePeriod>-1</validationDeadlinePeriod><validationPeriod>3780000</validationPeriod></ObtainLicenseResponse>"""
    xml = f"<!-- SHA512withRSA-{base64.b64encode(rsa1.sign(xml_content.encode(), 'SHA512')).decode()}-{cert1} -->\n{xml_content}"
    return XMLResponse(xml)
